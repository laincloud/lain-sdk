--yaml = require "lyaml"
--yaml = package.loadlib("yaml.so","yaml")
--yaml()
--print ("yaml2json package path " .. package.path)
--print ("yaml2json c path       " .. package.cpath)
...
function typeParser(info)
	...
	if type(info) ~= "string" then
		error("invalid proc type " .. info ,0)
	end

	if ProcType[info] == nil then
		error("invalid procc type" .. info,0 )
	end
...
---------------------------------------------------------------------------------------------------------------
function getJson(fileName, repo_name, meta_version)
	tab = yaml2json(fileName)
...
function checkProcsValid(tab)
	...
	for pk,pv in pairs(procs) do
		if type(pv['cpu']) ~= "number" then
			error("CPU number is invalid " .. pv['cpu'],0)
		end
		if type(pv['num_instances']) ~= "number" then
			error("instances number is invalid " .. pv['num_instances'],0)
		end
	end
end
...
function yaml2json(fileName)
	...
	print("\n-----------------------------")
	print("Going to convert "..fileName.. " to json.")
	print("-----------------------------")

	--data = yaml.load(str)
	---------------------------------------------------------------------------------input is json or yaml~
	yamlT = yaml.load(str)
	jsonStr = json.encode(yamlT)
	...
	if data["appname"] == nil then
		error("invalid lain conf: no appname",0)
	end
	...
	if data["build"] == nil then
		error("invalid lain conf: no build section in lain.yaml",0)
	end
	-- base must be in build
	build = data["build"]
	if build["base"] == nil then
		error("invalid lain conf: no base section in build",0)
	end
	-- build.script 
	...
	for k,v in pairs(typekey) do
		...
		if pos == nil then
			...
			if procs[t] ~= nil then
				error("duplicated proc name " .. t,0)
			end
		...
		---------------------------------------------------------------------------------------------------------------
			...
			--	print(json.encode(eachproc))
				if procs[n] ~= nil then
                    error("duplicated proc name " .. n,0)
                end
			...
			elseif t == "portal" or t == "worker" or t == "oneshot" or t == "web" or t == "service" then
				...
				if PreType ~= nil and ProcType[PreType] ~= localproc["type"] then
					error("Redefine proc type and they are different.",0)
				end
				...
				if t == "service" then
					...
					if data[v]["portal"] == nil then
						error("a service without define portal",0)
					end
